...
% --- Fonts (Approximating Word Defaults) ---
% Requires system fonts "Times New Roman" and "Arial" to be installed.
...
% --- Formatting ---
\usepackage{parskip} % Adds space between paragraphs, removes indentation (Word style)
\linespread{1.15}    % Approximate line spacing for Word defaults
...
\addbibresource{references.bib} % You must create this file
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small DVA305} % Course Code [cite: 2]
...
\begin{titlepage}
    ...
    {\Huge \bfseries \sffamily Titel på rapporten} \\ % [cite: 7]
    \vspace{0.5cm}
    {\large En undertitel om så önskas}
    
    \vspace{2cm}
    
    % Optional Image placeholder [cite: 13]
    \textit{[Plats för en väl vald bild]}
    
    \vspace{3cm}
    
    {\Large \textbf{Författare:}} \\ \vspace{0.2cm}
    Förnamn Efternamn \\ % [cite: 8]
    \texttt{epost@student.mdu.se} % [cite: 10]
    
    \vspace{2cm}
    
    {\Large \textbf{Kurs:}} \\ \vspace{0.2cm}
    DVA305 - Information – kunskap – vetenskap – etik \\ % [cite: 9]
    [VT/HT] 202X % [cite: 4]
    
    \vfill
    
    {\large Ort och datum:}\\ % [cite: 11]
...
% List of figures should be here according to instructions [cite: 30]
\listoffigures
\newpage
...
% ==========================================
% 5. FORSKNINGSETISKT STÄLLNINGSTAGANDE [cite: 72]
% ==========================================
...
% ==========================================
\section{Utförande}
\textit{(Obs: Ni ska inte använda rubriken ovan, utan ersätta den med lämplig rubrik beroende på ert arbete[cite: 77].)}
...
% ==========================================
\newpage
% This prints the bibliography. Make sure to have a references.bib file.
\printbibliography[heading=bibintoc, title={Referenser}]

\textit{Instruktioner för referenser:}
Referenser ska anges enligt IEEE-standarden. I den löpande texten anger ni referenserna med siffror (löpnummer) i hakparentes, t.ex. [1].
